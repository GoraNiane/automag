 ...
 */
function normalizeAndSaveImages(incomingImages, existingImages) {
 ...
 */
function checkAuthorization(req, vehicleId, callback) {
    if (!req.user) {
    ...
    db_1.default.get('SELECT sellerId FROM listings WHERE vehicleId = ?', [vehicleId], (err, row) => {
        if (err) {
...
router.get('/', (req, res) => {
    db_1.default.all('SELECT * FROM vehicles', [], (err, rows) => {
        if (err) {
...
router.get('/listings/all', (req, res) => {
    db_1.default.all('SELECT * FROM listings', [], (err, rows) => {
        if (err) {
            return res.status(500).json({ message: 'Erreur de base de données : ' + err.message });
        }
        res.json(rows);
...
router.post('/', auth_1.protect, (req, res) => {
    const { id, brand, model, year, price, mileage, fuel, transmission, bodyType, color, location, power, description, equipments, images, primaryImage, condition, availability } = req.body;
    ...
    db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [vehId], (err, existingVeh) => {
        if (err) {
            return res.status(500).json({ message: 'Erreur de base de données : ' + err.message });
        }
        ...
        `;
                ...
                db_1.default.run(sqlUpdate, paramsUpdate, function (errUpdate) {
                    if (errUpdate) {
                        return res.status(500).json({ message: 'Erreur lors de la mise à jour : ' + errUpdate.message });
                    }
                    // Publish listing
                    db_1.default.run(`UPDATE listings SET status = 'Publiée' WHERE vehicleId = ?`, [vehId], (errList) => {
                        if (errList) {
      ...
      `;
            ...
            db_1.default.run(sqlInsert, paramsInsert, function (errInsert) {
                if (errInsert) {
          ...
          VALUES (?, ?, ?, ?, ?, 0, 0)
        `;
                db_1.default.run(sqlList, [listId, vehId, req.user?.id || 'usr-admin', 'Publiée', new Date().toISOString()], (errList) => {
                    if (errList) {
...
router.put('/:id', auth_1.protect, (req, res) => {
    const { id } = req.params;
    const { brand, model, year, price, mileage, fuel, transmission, bodyType, color, location, power, description, equipments, images, primaryImage, condition, availability } = req.body;
    if (!brand || !model || !year || !price || !mileage) {
        return res.status(400).json({ message: 'Veuillez renseigner les informations obligatoires.' });
    }
    checkAuthorization(req, id, (authorized, statusCode, authMsg) => {
        ...
        db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [id], (errVeh, vehicle) => {
            if (errVeh) {
      ...
      `;
            ...
            db_1.default.run(sql, params, function (errRun) {
                if (errRun) {
...
router.delete('/:id', auth_1.protect, (req, res) => {
    const { id } = req.params;
    checkAuthorization(req, id, (authorized, statusCode, authMsg) => {
        if (!authorized) {
            return res.status(statusCode).json({ message: authMsg });
        }
        db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [id], async (err, vehicle) => {
            if (err) {
            ...
            db_1.default.run('DELETE FROM listings WHERE vehicleId = ?', [id], (errList) => {
                if (errList) {
                    return res.status(500).json({ message: 'Erreur lors de la suppression de l\'annonce.' });
                }
                db_1.default.run('DELETE FROM vehicles WHERE id = ?', [id], function (errVeh) {
                    if (errVeh) {
...
router.post('/:id/images', auth_1.protect, (req, res) => {
    upload.array('photos', 7)(req, res, async (multerErr) => {
        if (multerErr) {
            if (multerErr instanceof multer_1.default.MulterError && multerErr.code === 'LIMIT_FILE_SIZE') {
                return res.status(400).json({ message: 'Une des photos dépasse 10 Mo.' });
            }
            if (multerErr instanceof multer_1.default.MulterError && multerErr.code === 'LIMIT_UNEXPECTED_FILE') {
        ...
        try {
            ...
            db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [id], async (err, vehicle) => {
               ...
               VALUES (?, '', '', 0, 0, 0, 'Essence', 'Automatique', 'SUV', 'Dakar', ?, ?, 'Neuf', 'Disponible')`, [id, primaryImage, imagesStr], (errInsert) => {
                            ...
                            const listId = `lst-${Date.now()}`;
                            db_1.default.run(`INSERT INTO listings (id, vehicleId, sellerId, status, createdAt, views, isPromoted)
                   VALUES (?, ?, ?, 'Brouillon', ?, 0, 0)`, [listId, id, req.user?.id || 'usr-admin', new Date().toISOString()], (errList) => {
                                if (errList) {
                        ...
                        db_1.default.run(`UPDATE vehicles SET images = ?, primaryImage = ? WHERE id = ?`, [imagesStr, primaryImage, id], (errUpdate) => {
                            if (errUpdate) {
...
router.delete('/:id/images/:imageId', auth_1.protect, (req, res) => {
    ...
    checkAuthorization(req, id, (authorized, statusCode, authMsg) => {
        ...
        db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [id], async (err, vehicle) => {
            ...
            if (targetImage.publicId) {
                try {
                    await (0, cloudinary_1.deleteFromCloudinary)(targetImage.publicId);
                }
                catch (cloudinaryErr) {
...
router.put('/:id/images/reorder', auth_1.protect, (req, res) => {
    const { id } = req.params;
    ...
    checkAuthorization(req, id, (authorized, statusCode, authMsg) => {
        ...
        db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [id], (err, vehicle) => {
            if (err) {
                return res.status(500).json({ message: 'Erreur de base de données.' });
            }
            if (!vehicle) {
...
router.put('/:id/images/:imageId/primary', auth_1.protect, (req, res) => {
    const { id, imageId } = req.params;
    checkAuthorization(req, id, (authorized, statusCode, authMsg) => {
        if (!authorized) {
            return res.status(statusCode).json({ message: authMsg });
        ...
        db_1.default.get('SELECT * FROM vehicles WHERE id = ?', [id], (err, vehicle) => {
            ...
            db_1.default.run('UPDATE vehicles SET primaryImage = ? WHERE id = ?', [targetImage.url, id], (errUpdate) => {
                if (errUpdate) {
...
router.put('/listings/:listingId/status', auth_1.protect, (req, res) => {
    ...
    db_1.default.get('SELECT vehicleId FROM listings WHERE id = ?', [listingId], (errList, listing) => {
        if (errList || !listing) {
        ...
        checkAuthorization(req, listing.vehicleId, (authorized, statusCode, authMsg) => {
            ...
            db_1.default.run('UPDATE listings SET status = ? WHERE id = ?', [status, listingId], function (err) {
                if (err) {
                    return res.status(500).json({ message: 'Erreur de base de données : ' + err.message });
                }
