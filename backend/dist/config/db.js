...
async function initializeDatabase() {
    ...
    `);
        ...
        const [users] = await promisePool.query('SELECT * FROM users WHERE email = ?', [adminEmail]);
        const adminUser = users && users.length > 0 ? users[0] : null;
        const adminId = adminUser ? adminUser.id : 'usr-admin';
        if (!adminUser) {
            await promisePool.query(`INSERT INTO users (id, email, password, firstName, lastName, role, avatar, location, createdAt) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                adminId,
                adminEmail,
                hashedPassword,
            ...
            await promisePool.query('UPDATE users SET password = ?, role = ? WHERE email = ?', [hashedPassword, 'ADMIN', adminEmail]);
        }
        console.log('[AUTH] Admin synchronization completed');
        // Check vehicles count & seed
        const [vehiclesRow] = await promisePool.query('SELECT COUNT(*) as count FROM vehicles');
        const vehicleCount = vehiclesRow && vehiclesRow.length > 0 ? vehiclesRow[0].count : 0;
        if (vehicleCount === 0) {
            console.log('Seeding initial vehicles...');
            for (const v of initialVehicles) {
                await promisePool.query(`INSERT INTO vehicles (id, brand, model, year, price, mileage, fuel, transmission, bodyType, color, location, power, description, equipments, images, primaryImage, \`condition\`, isFeatured, isPromo, availability) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE brand=brand`, [
                ...
                await promisePool.query(`INSERT INTO listings (id, vehicleId, sellerId, status, createdAt, views, isPromoted) 
           VALUES (?, ?, ?, ?, ?, ?, ?)
