...
function seedData() {
    db.get('SELECT COUNT(*) as count FROM users', (err, row) => {
        ...
        if (!row || row.count === 0) {
            ...
            bcryptjs_1.default.hash('adminpassword', 10, (hashErr, hashedPassword) => {
                if (hashErr || !hashedPassword)
                    return console.error('Hashing password failed');
                db.run(`INSERT INTO users (id, email, password, firstName, lastName, role, avatar, location, createdAt) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                    adminId,
                    adminEmail,
                    hashedPassword,
                    ...
                    if (insertErr)
                        return console.error('Failed to insert seed admin user:', insertErr.message);
...
function insertSeedVehicles(adminId) {
    ...
    initialVehicles.forEach((v) => {
        db.run(`INSERT INTO vehicles (id, brand, model, year, price, mileage, fuel, transmission, bodyType, color, location, power, description, equipments, images, primaryImage, \`condition\`, isFeatured, isPromo, availability) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            if (errVeh)
                return console.error('Failed to seed vehicle:', errVeh.message);
            const status = v.status || 'Publiée';
            db.run(`INSERT INTO listings (id, vehicleId, sellerId, status, createdAt, views, isPromoted) 
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                `lst-${v.id.split('-')[1]}`,
                v.id,
                adminId,
                ...
                if (!errList && status === 'Vendue' && v.sale) {
                    const sale = v.sale;
                    db.run(`INSERT INTO sales (id, vehicleId, salePrice, buyerName, buyerPhone, saleDate, invoiceNumber)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                        `sal-${v.id.split('-')[1]}`,
                        v.id,
                        sale.salePrice,
